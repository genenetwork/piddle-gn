...
print("PIL_LOCATION:", Image.__file__)
...
if __name__ == '__main__':
	_truetypeprefix = os.path.join(os.curdir,'truetypefonts')
else:
	_truetypeprefix = os.path.join(os.path.split(__file__)[0],'truetypefonts')
...
class PILCanvas( Canvas ):
	...
	def stringWidth(self, s, font=None):
		...
		"""
		if not _widthmaps:
			print "warning no _widthmaps available"
	...
	#------------- drawing methods --------------
	def drawLine(self, x1,y1, x2,y2, color=None, width=None, clipX = [], clipY = []):
		...
		##clipping
		##Currently only clip for X-Axis
		##Will implement Sutherland - Cohen clipping algorithm
		##if necessary
		if clipX:
			if x2 < x1:
				temp = x2
			...
			if ((x1 < minX and x2 < minX) or (x1 > maxX and x2 > maxX)):
				return
			elif ((x1 >= minX and x1 <= maxX) and (x2 >= minX and x2 <= maxX)):
				pass
			elif (x1 < minX):
				y1 = y2 - float(y2-y1)*(x2-minX)/(x2-x1)
				x1 = minX
				if x2 > maxX:
					y2 = y2 - float(y2-y1)*(x2-maxX)/(x2-x1)
					x2 = maxX
			elif (x2 > maxX):
				y2 = y2 - float(y2-y1)*(x2-maxX)/(x2-x1)
				x2 = maxX
		...
		if w > 1:
			...
			if Image.VERSION <= "1.0":
				...
				self._pen.line( (x1,y1, x2,y2) )
			elif Image.VERSION <= "1.1.1":
				self._pen.line( (x1,y1, x2,y2) )
			else:
				self._pen.line((x1,y1,x2,y2))

	def drawPolygon(self, pointlist, 
				edgeColor=None, edgeWidth=None, fillColor=None, closed=0, clipX = [], clipY = []):
		...
		if (closed or (pts[0][0]==pts[-1][0] and pts[0][1]==pts[-1][1])) \
			...
			for p in pts[1:]:
				self.drawLine(oldp[0],oldp[1], p[0],p[1], edgeColor, edgeWidth, clipX, clipY)
				oldp = p
	...
	def drawString(self, s, x,y, font=None, color=None, angle=0):
		...
		tempsize = max(sWidth*4, sHeight*2.5)
		...
		if angle:
			...
			tempimg = tempimg.rotate( angle, Image.NEAREST )
			temppen = ImageDraw.ImageDraw(tempimg)
	...
	def drawCross(self,x,y,color=None,size=10,width=None):
		self.drawLine(x-size,y,x+size,y,color,width)
		self.drawLine(x,y-size,x,y+size,color,width)
	
	def drawXCross(self,x,y,color=None,size=10,width=None):
		self.drawLine(x+size,y+size,x-size,y-size,color,width)
		self.drawLine(x-size,y+size,x+size,y-size,color,width)
